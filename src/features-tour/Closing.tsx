  ...
  return (
    <>
      ...
      <section id="cta" className="bf-section" style={{ paddingTop: 40 }}>
        <div className="bf-wrap">
          <Reveal>
            <div style={{ position: 'relative', overflow: 'hidden', borderRadius: 32, padding: '64px 40px', textAlign: 'center', background: 'linear-gradient(160deg, #423B33, #2E2924)', color: '#F3ECE2' }}>
              <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: 'radial-gradient(600px 300px at 50% 0%, rgba(198,172,138,.28), transparent 70%)' }} />
              <div style={{ position: 'relative' }}>
                <span className="bf-pill" style={{ background: 'rgba(243,236,226,.12)', color: '#E4D6C2', marginBottom: 20 }}><Sparkles style={{ width: 13, height: 13 }} /> You've seen it. Now build with it.</span>
                <h2 style={{ color: '#FBF6EF', fontSize: 'clamp(32px,4.4vw,54px)', lineHeight: 1.05 }}>Start your project on BYLD</h2>
                <p style={{ color: '#C9BEAE', fontSize: 19, maxWidth: 520, margin: '18px auto 0', lineHeight: 1.55 }}>
                  Spin up the Whitefield Villa workspace — or your own — and put every one of these features to work today.
                </p>
                <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 30, flexWrap: 'wrap' }}>
                  <Link to="/login" className="bf-btn" style={{ background: '#F3ECE2', color: '#2E2924', textDecoration: 'none' }}>Start free trial <ArrowRight style={{ width: 16, height: 16 }} /></Link>
                  <button onClick={openDemo} className="bf-btn" style={{ background: 'rgba(243,236,226,.1)', color: '#F3ECE2', boxShadow: 'inset 0 0 0 1px rgba(243,236,226,.25)' }}>Book a live demo</button>
                </div>
                <div style={{ marginTop: 22, fontSize: 13, color: '#A99C8B', display: 'flex', gap: 18, justifyContent: 'center', flexWrap: 'wrap' }}>
      ...
      <footer style={{ borderTop: '1px solid var(--bf-line)', padding: '40px 0' }}>
        <div className="bf-wrap" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
          <span style={{ fontWeight: 800, letterSpacing: '-.03em', fontSize: 22, color: 'var(--bf-ink)', display: 'inline-flex', alignItems: 'baseline', gap: 7 }}>
            BYLD<span style={{ color: 'var(--bf-muted-2)', fontWeight: 500 }}>/</span><span style={{ color: '#8C8279', fontWeight: 600, letterSpacing: '.22em', fontSize: 13, textTransform: 'uppercase' }}>Space</span>
          </span>
