...
import { HardDrive, CheckCircle2, AlertCircle, Link as LinkIcon, Unlink } from 'lucide-react';
...
export default function DriveIntegration() {
  ...
  const handleConnect = async () => {
    try {
      // In a real env, invoke the edge function.
      if (!user) return;
      ...
      toast.error('Failed to initiate Google Drive connection. Ensure Edge Functions are deployed.');
      console.error(error);
    }
  };
  ...
  return (
    <div className="border border-border/60 rounded-xl p-5 bg-card relative overflow-hidden mt-6">
      <div className="absolute -right-6 -top-6 w-24 h-24 bg-[#4285F4]/10 rounded-full blur-2xl pointer-events-none" />
      <div className="absolute -right-6 -bottom-6 w-24 h-24 bg-[#0F9D58]/10 rounded-full blur-2xl pointer-events-none" />
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              {connection 
                ? `Syncing as ${connection.google_account_email}`
                : "Import and sync project documents directly from Google Drive."}
            </p>
          </div>
        </div>
