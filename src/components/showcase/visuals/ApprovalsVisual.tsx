...
export default function ApprovalsVisual({ progress }: VisualProps) {
  ...
  return (
    <div className={`bg-white border border-[#e1e7e2] rounded-[24px] overflow-hidden ${STAGE_SHADOW}`}>
      <div className="relative h-[188px] bg-[#f1f4f1]">
        <img src={PHOTO_BATHROOM} alt="Fixture" loading="lazy" className="w-full h-full object-cover" />
        <motion.span
          style={{ opacity: reqOpacity, x: reqX }}
          className="absolute left-3.5 top-3.5 flex items-center gap-2 pl-1.5 pr-3 py-1.5 rounded-full text-white text-[12px] font-medium bg-[#121413]/60 backdrop-blur-[8px]"
        >
          <span className="w-[22px] h-[22px] rounded-full grid place-items-center text-[10px] font-bold bg-[#6e8a72]">JD</span>
          Requested by John D.
        </motion.span>
      </div>
          ...
          {result === null ? (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <h4 className="text-[19px] font-bold tracking-[-0.02em] text-[#141714]">Matte Black Basin Faucet</h4>
                <span className="bg-[#e8f1ea] text-[#6b986d] text-[14px] font-bold px-[11px] py-1 rounded-full whitespace-nowrap">+$450</span>
              </div>
              <p className="text-[14px] text-[#8a8f86] mt-2 leading-[1.5]">
              ...
              <div className="grid grid-cols-2 gap-3 mt-5">
                ...
                <button
                  ...
                  className="py-3.5 rounded-[12px] text-[15px] font-semibold text-[#353a36] bg-[#f1f4f1] transition hover:bg-[#e1e7e2] active:scale-[0.97]"
                >
                  Reject
                </button>
              </div>
            ...
            >
              <div
                className={`w-14 h-14 mx-auto mb-3.5 rounded-full grid place-items-center ${
                  result === 'approved' ? 'bg-[#e6f6ec] text-[#16a34a]' : 'bg-[#e8f1ea] text-[#6b986d]'
                }`}
              >
                {result === 'approved' ? <Check className="w-[26px] h-[26px]" strokeWidth={2.6} /> : <X className="w-[26px] h-[26px]" strokeWidth={2.6} />}
              </div>
              <h4 className="text-[19px] font-bold text-[#141714]">{result === 'approved' ? 'Approved' : 'Rejected'}</h4>
              <p className="text-[13px] text-[#8a8f86] mt-1.5">
                {result === 'approved' ? 'Signed off by you · just now' : 'Reason capture required before closing'}
              </p>
              <div className="mt-4 pt-3.5 border-t border-[#edf2ee] flex flex-col gap-[9px] text-left">
                {TRAIL[result].map((t) => (
                  <div key={t} className="flex items-center gap-[9px] text-[12px] text-[#8a8f86]">
                    <span className="w-1.5 h-1.5 rounded-full flex-none bg-[#bcd0bf]" />
                    {t}
                  </div>
                ))}
              </div>
              <button
                ...
                className="mt-4 text-[12px] font-semibold text-[#6e8a72] hover:text-[#4e6b57] transition-colors"
              >
                Reset demo
              </button>
            </motion.div>
