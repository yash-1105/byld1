...
const BAR: Record<TimelineTask['cls'], string> = {
  ...
  prog: 'bg-[#e8f1ea] text-[#6b986d]',
  pend: 'bg-[#f1f4f1] text-[#8a8f86]',
...
function TaskRow({ t, i, progress }: { t: TimelineTask; i: number; progress: MotionValue<number> }) {
  ...
  return (
    <div className="grid grid-cols-[120px_1fr] items-center gap-3.5 py-[13px] border-b border-[#edf2ee]">
      <span className="text-[14px] font-semibold text-[#141714]">{t.n}</span>
      <div className="relative h-[30px]">
        <div className="absolute inset-0 grid grid-cols-4">
          {[0, 1, 2, 3].map((c) => (
            <i key={c} className={c < 3 ? 'border-r border-dashed border-[#e1e7e2]' : ''} />
          ))}
        </div>
        <motion.div
          style={{
...
export default function TimelineVisual({ progress }: VisualProps) {
  ...
  return (
    <div className={`bg-white border border-[#e1e7e2] rounded-[24px] overflow-hidden p-[22px] ${STAGE_SHADOW}`}>
      <div className="grid grid-cols-[120px_repeat(4,1fr)] pb-3 border-b border-[#edf2ee] text-[11px] font-bold tracking-[0.08em] uppercase text-[#8a8f86]">
        <span>Task</span>
        <span>W1</span>
        <span>W2</span>
        <span>W3</span>
